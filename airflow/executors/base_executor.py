...
class BaseExecutor(LoggingMixin):
    ...
    def queue_command(self,
        ...
        if simple_task_instance.key not in self.queued_tasks and simple_task_instance.key not in self.running:
            ...
            heappush(self.queued_tasks_priority_queue, (-priority, (command, priority, queue, simple_task_instance)))
        else:
    ...
    def trigger_tasks(self, open_slots: int) -> None:
        ...
        for _ in range(min((open_slots, len(self.queued_tasks)))):
            key, (command, _, _, simple_ti) = heappop(self.queued_tasks_priority_queue)
